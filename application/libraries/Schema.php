...
class Schema {
    ...
    static public function create_table($table_name, $callback,$forge = null) {
        $table_definition = new Schema_Table_Definition($table_name,$forge);
...
class Schema_Table_Definition {
    ...
    public function __construct($table_name = '', $forge = '') {
    ...
    public function create_table() {
        ...
        if (empty($this->forge)) {
            ...
            $castIron = $ci->load->dbforge($this->forge,true);
        }
        
        //$ci->dbforge->add_field($this->columns());
        $castIron->add_field($this->columns());
        
        foreach ($this->keys() as $key => $primary) {
            //$ci->dbforge->add_key($key, $primary);
            $castIron->add_key($key,$primary);
        }
        
        //$ci->dbforge->create_table($this->table_name());
        $castIron->create_table($this->table_name());
    }
    ...
    public function bigint($column_name,$options = array()) {
        $this->add_definition_rule($column_name, array(
            'type' => 'BIGINT'
        ), $options);
    }
    ...
    public function auto_increment_integer($column_name, $options = array()) {
        $this->bigint($column_name, array_merge(array(
            'unsigned' => TRUE,
            'auto_increment' => TRUE
        ), $options));
